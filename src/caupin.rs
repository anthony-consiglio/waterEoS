...
// ═══════════════════════════════════════════════════════════════════════════
// Parameter sets (caupin_eos/params.py = Table III, params_kim.py = Table II)
// ═══════════════════════════════════════════════════════════════════════════

/// Full parameter set for the Caupin & Anisimov (2019) two-state EoS.
/// Two instances exist: PARAMS (Table III, without Kim et al. kT data —
/// the published preferred fit) and PARAMS_KIM (Table II, with Kim data).
pub struct CaupinParams {
    pub r: f64, pub m_h2o: f64,
    pub tc: f64, pub pc: f64, pub vc: f64, pub p_scale_mpa: f64,
    pub omega0: f64,
    pub lam: f64, pub a: f64, pub b: f64, pub d: f64, pub f: f64,
    pub a0: f64, pub a1: f64,
    pub c01: f64, pub c02: f64, pub c11: f64, pub c20: f64,
    pub c03: f64, pub c12: f64, pub c21: f64, pub c30: f64,
    pub c04: f64, pub c13: f64, pub c22: f64, pub c40: f64, pub c14: f64,
    pub ps_a: f64, pub ps_b: f64, pub ps_c: f64, pub ps_t0: f64,
    pub s_offset: f64, pub h_offset: f64,
}

const R_GAS: f64 = 8.314462;
const M_WATER: f64 = 0.018015268;

/// Table III — without Kim et al. kT data (default, erratum-corrected).
pub const PARAMS: CaupinParams = CaupinParams {
    r: R_GAS, m_h2o: M_WATER,
    tc: 218.1348, pc: 71.94655, vc: 18.22426e-6,
    p_scale_mpa: R_GAS * 218.1348 / 18.22426e-6 / 1e6,
    omega0: 0.1854443,
    lam: 1.653737, a: 0.1030250, b: -0.0392417, d: -0.01039947, f: 1.021512,
    a0: -0.08118730, a1: 0.05070641,
    c01: 1.126869, c02: 0.01005341, c11: -0.2092770, c20: -2.520114,
    c03: -0.001149367, c12: -0.008992042, c21: 0.2118502, c30: 0.1087670,
    c04: 0.00007573062, c13: 0.002393927, c22: -0.01831198, c40: 0.02803712,
    c14: -0.0001641608,
    ps_a: -462.0, ps_b: 2.61, ps_c: -0.0065, ps_t0: 182.0,
    s_offset: -146.1582559570, h_offset: 112500.2342686583,
};

/// Table II — with Kim et al. kT data (as printed in the 2019 paper; the
/// 2025 erratum corrects Table III only, so Table II signs are unchanged).
pub const PARAMS_KIM: CaupinParams = CaupinParams {
    r: R_GAS, m_h2o: M_WATER,
    tc: 219.4717, pc: 58.73897, vc: 18.74395e-6,
    p_scale_mpa: R_GAS * 219.4717 / 18.74395e-6 / 1e6,
    omega0: 0.2931166,
    lam: 2.701194, a: 0.06424121, b: -0.05480673, d: -0.007881021, f: 0.5910234,
    a0: -0.08666044, a1: 0.1116703,
    c01: 1.113806, c02: 0.01422090, c11: -0.3451214, c20: -1.040621,
    c03: -0.001587632, c12: -0.03211844, c21: 0.2203273, c30: -1.290837,
    c04: 0.0001027115, c13: 0.003822330, c22: -0.01169451, c40: 0.5408265,
    c14: -0.0002424047,
    ps_a: -462.0, ps_b: 2.61, ps_c: -0.0065, ps_t0: 182.0,
    s_offset: 294.5902568285, h_offset: 201073.8069705671,
};
...
fn spinodal_pressure(t_k: f64, prm: &CaupinParams) -> (f64, f64, f64) {
    let (PS_A, PS_B, PS_C, PS_T0) = (prm.ps_a, prm.ps_b, prm.ps_c, prm.ps_t0);
    let dt = t_k - PS_T0;
...
#[inline(always)]
#[allow(non_snake_case)]
fn compute_gsigma(d_th: f64, d_ph: f64, t_k: f64, prm: &CaupinParams) -> (f64, f64, f64, f64, f64, f64) {
    let (A0, A1, PC, TC, P_SCALE_MPA) = (prm.a0, prm.a1, prm.pc, prm.tc, prm.p_scale_mpa);
    let ah = A0 + A1 * d_th;
    let dah_dt = A1;
    let (ps, dps_dt, d2ps_dt2) = spinodal_pressure(t_k, prm);
    ...
    // Below the liquid-vapor spinodal (u <= 0) the model is undefined; propagate NaN.
    let u_safe = if u > 0.0 { u } else { f64::NAN };
    let sqrt_u = u_safe.sqrt();
    let u32 = u_safe * sqrt_u;
    let u12 = sqrt_u;
    let u_m12 = 1.0 / sqrt_u;
    let gs = ah * u32;
...
#[inline(always)]
#[allow(non_snake_case)]
fn compute_ga_poly(d_th: f64, d_ph: f64, prm: &CaupinParams) -> (f64, f64, f64, f64, f64, f64) {
    let (C01, C02, C11, C20, C03, C12, C21, C30, C04, C13, C22, C40, C14) = (
        prm.c01, prm.c02, prm.c11, prm.c20, prm.c03, prm.c12, prm.c21, prm.c30,
        prm.c04, prm.c13, prm.c22, prm.c40, prm.c14);
    let t = d_th;
    let q = d_ph;
...
#[inline(always)]
#[allow(non_snake_case)]
fn compute_gba(d_th: f64, d_ph: f64, prm: &CaupinParams) -> (f64, f64, f64, f64, f64, f64) {
    let (LAM, A_PARAM, B_PARAM, D_PARAM, F_PARAM) = (prm.lam, prm.a, prm.b, prm.d, prm.f);
...
#[inline(always)]
#[allow(non_snake_case)]
fn compute_omega(d_th: f64, d_ph: f64, prm: &CaupinParams) -> (f64, f64, f64, f64, f64, f64) {
    let OMEGA0 = prm.omega0;
    let th = 1.0 + d_th;
...
fn physical_props(
    ...
    t_k: f64, g_hat: f64, prm: &CaupinParams,
) -> (f64, f64, f64, f64, f64, f64, f64, f64, f64, f64) {
    let (R, M_H2O, TC, VC) = (prm.r, prm.m_h2o, prm.tc, prm.vc);
    let th = t_k / TC;
    let v_molar = VC * vh;
    let v_spec = v_molar / M_H2O;
    // Below the liquid-vapor spinodal, vh / v_spec become NaN. Standard `> 0.0`
    // comparisons against NaN are `false`, which would otherwise convert NaN
    // into Infinity here. Preserve NaN by checking `is_nan()` first.
    let rho = if v_spec.is_nan() { f64::NAN }
              else if v_spec > 0.0 { 1.0 / v_spec }
              else { f64::INFINITY };
    let s_molar = R * sh_red;
    let s_spec = s_molar / M_H2O;
    let cp_molar = -R * th * d2g_dt2;
    let cp = cp_molar / M_H2O;
    let kappa_t = if vh.is_nan() { f64::NAN }
                  else if vh.abs() > 1e-30 { -(VC / (R * TC)) * d2g_dp2 / vh }
                  else { f64::INFINITY };
    let kt = if kappa_t.is_nan() { f64::NAN }
             else if kappa_t.abs() > 1e-30 && kappa_t.is_finite() { 1.0 / kappa_t / 1e6 }
             else { 0.0 };
    let alpha = if vh.is_nan() { f64::NAN }
                else if vh.abs() > 1e-30 { (1.0 / TC) * d2g_dpdt / vh }
                else { 0.0 };
    let cv = if kappa_t.is_nan() || alpha.is_nan() { f64::NAN }
             else if kappa_t > 0.0 && kappa_t.is_finite() {
                 cp - t_k * v_spec * alpha * alpha / kappa_t
             } else { cp };
    let kappa_s = if kappa_t.is_nan() || alpha.is_nan() { f64::NAN }
                  else if cp > 0.0 { kappa_t - t_k * v_spec * alpha * alpha / cp }
                  else { kappa_t };
    let ks = if kappa_s.is_nan() { f64::NAN }
             else if kappa_s > 0.0 { 1.0 / kappa_s / 1e6 }
             else { f64::INFINITY };
    let vel = if rho.is_nan() || kappa_s.is_nan() { f64::NAN }
              else if rho > 0.0 && kappa_s > 0.0 { (1.0 / (rho * kappa_s)).sqrt() }
              else { f64::NAN };
...
#[inline]
#[allow(non_snake_case)]
fn compute_point(t_k: f64, p_mpa: f64, prm: &CaupinParams) -> PointResult {
    let (TC, PC, P_SCALE_MPA, OMEGA0, S_OFFSET, H_OFFSET) =
        (prm.tc, prm.pc, prm.p_scale_mpa, prm.omega0, prm.s_offset, prm.h_offset);
    let d_th = (t_k - TC) / TC;
    let d_ph = (p_mpa - PC) / P_SCALE_MPA;
    let th = 1.0 + d_th;

    let (gs, dgs_dp, dgs_dt, d2gs_dp2, d2gs_dt2, d2gs_dpdt) = compute_gsigma(d_th, d_ph, t_k, prm);
    let (gp, dgp_dp, dgp_dt, d2gp_dp2, d2gp_dt2, d2gp_dpdt) = compute_ga_poly(d_th, d_ph, prm);
    ...
    let (gba, dgba_dp, dgba_dt, d2gba_dp2, d2gba_dt2, d2gba_dpdt) = compute_gba(d_th, d_ph, prm);
    let (om, _dom_dp, _dom_dt, _d2om_dp2, _d2om_dt2, _d2om_dpdt) = compute_omega(d_th, d_ph, prm);
    ...
    let (rho, v, mut s, mut g, cp, cv, kt, ks, alpha, vel) =
        physical_props(vh, sh_red, d2g_dp2_total, d2g_dt2_total, d2g_dpdt_total, t_k, g_hat_mix, prm);
    let (rho_a, v_a, mut s_a, mut g_a, cp_a, cv_a, kt_a, ks_a, alpha_a, vel_a) =
        physical_props(dga_dp, -dga_dt, d2ga_dp2, d2ga_dt2, d2ga_dpdt, t_k, g_hat_a, prm);
    ...
    let (rho_b, v_b, mut s_b, mut g_b, cp_b, cv_b, kt_b, ks_b, alpha_b, vel_b) =
        physical_props(vhb, shb_red, d2gb_dp2, d2gb_dt2, d2gb_dpdt, t_k, g_hat_b, prm);
...
fn caupin_compute_batch<'py>(
    ...
    // Parallel per-point compute. Each (T, P) point is independent, so
    // rayon work-stealing scales linearly with available cores. The
    // sequential unpack that follows is pure memory writes — cheap.
    // py.allow_threads releases the GIL during the parallel block so
    // other Python threads (e.g. an awaiting FastAPI worker) keep
    // making progress.
    let points: Vec<PointResult> = py.allow_threads(|| {
        (0..n)
            .into_par_iter()
            .map(|i| compute_point(t_data[i], p_data[i], prm))
            .collect()
    });
...
/// Caupin (2019) Table III fit — without Kim et al. kT data (default).
#[pyfunction]
pub fn compute_batch_caupin<'py>(
    ...
    caupin_compute_batch(py, t_k, p_mpa, &PARAMS)
}

/// Caupin (2019) Table II fit — with Kim et al. kT data.
#[pyfunction]
pub fn compute_batch_caupin_kim<'py>(
    py: Python<'py>,
    t_k: PyReadonlyArray1<'py, f64>,
    p_mpa: PyReadonlyArray1<'py, f64>,
) -> PyResult<Bound<'py, PyDict>> {
    caupin_compute_batch(py, t_k, p_mpa, &PARAMS_KIM)
}
