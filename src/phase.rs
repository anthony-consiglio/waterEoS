...
// Caupin with-Kim fit (Table II). Only the LLCP-reduction params differ;
// the spinodal/binodal solver itself is shared via the generic impls.
mod caupin_kim_consts {
    pub const TC: f64 = 219.4717;
    pub const PC: f64 = 58.73897;
    const R: f64 = 8.314462;
    const VC: f64 = 18.74395e-6;
    pub const P_SCALE_MPA: f64 = R * TC / VC / 1e6;
    pub const OMEGA0: f64 = 0.2931166;
    pub const LAM: f64 = 2.701194;
    pub const A: f64 = 0.06424121;
    pub const B: f64 = -0.05480673;
    pub const D: f64 = -0.007881021;
    pub const F: f64 = 0.5910234;
}

struct CaupinKimPhase;

impl PhaseMath for CaupinKimPhase {
    #[inline]
    fn mixing_params(t_k: f64, p_mpa: f64) -> (f64, f64, f64) {
        use caupin_kim_consts::*;
        let d_th = (t_k - TC) / TC;
        let d_ph = (p_mpa - PC) / P_SCALE_MPA;
        let gba = LAM
            * (d_th + A * d_ph + B * d_th * d_ph + D * d_ph * d_ph + F * d_th * d_th);
...
#[pyfunction]
pub fn compute_spinodal_caupin_kim<'py>(
    py: Python<'py>,
    p_arr: PyReadonlyArray1<'py, f64>,
    t_llcp: f64,
    p_llcp: f64,
    ...
    let p = p_arr.to_vec().unwrap_or_else(|_| p_arr.as_slice().unwrap().to_vec());
    let result = compute_spinodal_impl::<CaupinKimPhase>(&p, t_llcp, p_llcp);
    result.to_py_dict(py)
}
...
#[pyfunction]
pub fn compute_binodal_caupin_kim<'py>(
    py: Python<'py>,
    spin_p: PyReadonlyArray1<'py, f64>,
    spin_t_upper: PyReadonlyArray1<'py, f64>,
    spin_t_lower: PyReadonlyArray1<'py, f64>,
    ...
    let tl = spin_t_lower
        .to_vec()
        .unwrap_or_else(|_| spin_t_lower.as_slice().unwrap().to_vec());
    let result = compute_binodal_impl::<CaupinKimPhase>(&sp, &tu, &tl, t_llcp, p_llcp);
    result.to_py_dict(py)
}
