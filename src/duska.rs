...
fn compute_va(ph: f64, ps: f64, vs: f64, b: f64) -> (f64, f64, f64) {
    // Below the liquid-vapor spinodal (radicand < 0) the model is undefined;
    // propagate NaN (matches duska_eos/core.py).
    let u_arg = 1.0 - ph / ps;
    let u = if u_arg >= 0.0 { u_arg.sqrt() } else { f64::NAN };
    let denom = u + b;
    let va = vs * b / denom;
    let dva_dp = if u.is_nan() { f64::NAN }
                 else if u > 1e-30 { vs * b / (2.0 * ps * denom * denom * u) }
                 else { f64::INFINITY };
    (va, dva_dp, u)
}
...
fn to_physical(
    ...
    // Below the spinodal vh / v_phys become NaN — preserve NaN here rather
    // than letting the `> 0.0` comparison silently map it to +Infinity
    // (matches duska_eos/core.py vectorized branch).
    let rho = if v_phys.is_nan() { f64::NAN }
              else if v_phys > 0.0 { 1.0 / v_phys }
              else { f64::INFINITY };
    let s_phys = sh * R_SPEC;
    let cp = R_SPEC * th * dsh_dt;

    let kappa_t_red = if dvh_dp.is_nan() || vh.is_nan() { f64::NAN }
                      else if dvh_dp.abs() > 1e-30 { -(1.0 / vh) * dvh_dp }
                      else { f64::INFINITY };
    let kappa_t = kappa_t_red / (P_VLCP * 1e6);
    let kt = if kappa_t.is_nan() { f64::NAN }
             else if kappa_t.abs() > 1e-30 && kappa_t.is_finite() {
                 1.0 / kappa_t / 1e6
             } else { 0.0 };

    let alpha = if vh.is_nan() { f64::NAN }
                else if vh > 0.0 { (1.0 / vh) * dvh_dt / T_VLCP }
                else { 0.0 };
    ...
    let kappa_s = if kappa_t.is_nan() || alpha.is_nan() { f64::NAN }
                  else if cp > 0.0 { kappa_t - t * v_phys * alpha * alpha / cp }
                  else { kappa_t };
...
pub fn compute_batch_duska<'py>(
    ...
    // Parallel per-point compute; see caupin.rs for the rationale.
    let points: Vec<PointResult> = py.allow_threads(|| {
        (0..n)
            .into_par_iter()
            .map(|i| compute_point(t_data[i], p_data[i]))
            .collect()
    });
