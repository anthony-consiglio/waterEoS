...
from watereos_gui.backend.computation import compute_multi_model_curves
...
def register(app):
    # Update property dropdown when model selection changes
    @app.callback(
        [Output('mc-property', 'options'),
...
def _build_overlay(results, model_keys, prop_key, settings):
    """All models on a single plot, color-coded by model."""
