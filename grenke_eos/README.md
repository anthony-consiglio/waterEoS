...
This model takes a purely empirical approach: a modified Tait-Tammann equation directly correlates the specific volume as a function of temperature and pressure, with no underlying physical model of HDL/LDL structural states. All thermodynamic properties are obtained analytically from the specific volume equation and a reference heat capacity, via exact analytical derivatives and path integration.

The model has 17 adjustable parameters and achieves an overall R^2 = 0.9991 against experimental data within its fitted range. Because it is purely empirical, it does not predict phase behavior (no LLCP, spinodals, or LDL fraction), and extrapolation beyond the fitted range is less reliable.
