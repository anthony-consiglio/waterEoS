...
def compute_VA(ph, pS, VS, B):
    ...
    # Below the liquid-vapor spinodal (radicand < 0) the model is undefined; propagate NaN.
    arg_u = 1.0 - ph / pS
    u = math.sqrt(arg_u) if arg_u >= 0.0 else float('nan')
    denom = u + B
...
def compute_batch(T_K, p_MPa):
    ...
    # ── (c) State A volume ──────────────────────────────────────────────
    # Propagate NaN below the spinodal rather than silently clamping.
    arg_u = 1.0 - ph / pS
    with np.errstate(invalid='ignore'):
        u = np.sqrt(np.where(arg_u >= 0.0, arg_u, np.nan))
    denom_VA = u + B
    VA = VS * B / denom_VA
    ...
    for node, weight in zip(nodes, weights):
        ...
        with np.errstate(invalid='ignore'):
            arg_u_i = 1.0 - ph_i / pS
            u_i = np.sqrt(np.where(arg_u_i >= 0.0, arg_u_i, np.nan))
        VA_i = VS * B / (u_i + B)
        g_P_leg += weight * VA_i
    g_P_leg *= half_ph
    ...
    def _phys_vec(Vh_v, Sh_v, dVh_dp_v, dVh_dT_v, dSh_dT_v, Th_v, ph_v,
        ...
        # Preserve NaN (below-spinodal) rather than mapping it to +inf.
        with np.errstate(divide='ignore', invalid='ignore'):
            rho = np.where(V_phys > 0, 1.0 / V_phys,
                           np.where(np.isnan(V_phys), np.nan, np.inf))
