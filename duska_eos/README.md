...
This model mixes two structural states at the volume level (volume-additive mixing):

```
V(T, P) = (1 - x) V_A(T, P) + x V_B(T, P)
```

where `V_A` and `V_B` are the specific volumes of the HDL and LDL states, and `x` is the equilibrium LDL fraction. The volume-additive formulation naturally captures the density anomaly and gives state A properties an analytic structure built directly from the spinodal curves.
...
- **Temperature:** 200-370 K
- **Pressure:** approximately -150 to 400 MPa

Because it is formulated above the liquid spinodal, the model extends into the stretched (negative-pressure) regime. Per Duska (2020), it reproduces the experimental data accurately at positive pressures up to about 100 MPa, with agreement gradually degrading at negative pressures.
...
- **Mixing and reference point:** Volume-additive mixing of the two states; reduced variables are referenced to the vapor-liquid critical point (647 K, 22 MPa), with the liquid-liquid critical point emerging as a prediction.
