...
def _load_precomputed(model_key):
    ...
    try:
        ...
        logger.debug('No precomputed phase diagram for %s', model_key, exc_info=True)
        return None
...
def compute_phase_diagram_data(model_key, n_pressures=150,
    ...
    # Build a cache key that includes all parameters so different kwargs
    # produce separate cache entries (and don't return stale precomputed data)
    params = (n_pressures, P_max, P_llcp_offset, tmd_n_T_scan, tmd_T_lo,
              tmd_bisection_iters, widom_slope, widom_window, widom_n_T_scan)
    default_params = (150, d['P_max'], d['P_llcp_offset'], d['tmd_n_T_scan'],
                      d['tmd_T_lo'], d['tmd_bisection_iters'], d['widom_slope'],
                      d['widom_window'], d['widom_n_T_scan'])
    cache_key = (model_key, params)
    using_defaults = (params == default_params)

    # Check caches: precomputed JSON is only valid for default parameters
    if cache_key in _phase_diagram_cache:
        return _phase_diagram_cache[cache_key]

    if using_defaults:
        precomputed = _load_precomputed(model_key)
        if precomputed is not None:
            _phase_diagram_cache[cache_key] = precomputed
            return precomputed
    ...
    elif model_key == 'caupin2019_kim':
        import functools
        from caupin_eos import params_kim as _pk
        from caupin_eos import phase_diagram as _cpd
        find_LLCP = functools.partial(_cpd.find_LLCP, _pk)
        compute_spinodal_curve = functools.partial(_cpd.compute_spinodal_curve, pset=_pk)
        compute_binodal_curve = functools.partial(_cpd.compute_binodal_curve, pset=_pk)
    else:
    ...
    _phase_diagram_cache[cache_key] = result
    return result
...
def _get_compute_batch(model_key):
    ...
    if model_key == 'caupin2019':
        ...
        from caupin_eos.core import compute_batch as _cb
        from caupin_eos import params_kim as _pk
...
def _compute_tmd_curve(model_key, llcp=None, n_points=80,
    ...
    try:
        ...
        logger.warning('TMD computation failed for %s', model_key, exc_info=True)
        return None
...
def _compute_widom_line(model_key, llcp, n_points=60,
    ...
    try:
        ...
        logger.warning('Widom line computation failed for %s', model_key, exc_info=True)
        return None
...
def _compute_kauzmann_curve(model_key, P_range=(0.0, 300.0), n_points=80,
    ...
    # Native Rust SeaFreeze: returns a SimpleNamespace with the same .S
    # attribute as upstream. tdvSpec narrowing isn't needed — the Rust
    # evaluator computes all properties from the same six base derivatives
    # in one pass, so asking for just S is no faster than getting all
    # properties.
    try:
        ice_out = _sf_grid('Ih', P_pts, T_scan)
    ...
    # T_target selects which branch to track. Vectorized over all
    # pressures: replaces an O(n_points * n_scan) double-loop.
    left, right = dS[:, :-1], dS[:, 1:]
    crossings = np.isfinite(left) & np.isfinite(right) & (left * right < 0)
    T_mid_all = 0.5 * (T_scan[:-1] + T_scan[1:])
    ...
    P_b = P_pts[has_crossing]

    # --- Pre-compute ice-Ih entropy interpolant from the existing grid ---
    # The bisection was previously calling SeaFreeze in scatter mode at
    # every iteration. SeaFreeze grid-mode evaluation on (P_pts, T_scan)
    # already happened at line ~616 to build the S_ice array; we reuse
    # those values via linear interpolation on T instead of re-evaluating.
    #
    # Accuracy: T_scan spacing is ~(T_scan_hi - T_scan_lo) / n_scan ≈ 0.8 K.
    # Linear interp error for ice-Ih S(T) (smooth, second derivative ~1e-2
    # J/(kg·K)/K^2) is h^2 * S'' / 8 ~ 1e-3 J/(kg·K). With d(dS)/dT ~ Cp/T
    # ~ 10 J/(kg·K)/K, this translates to ~1e-4 K uncertainty in T_kauz —
    # five orders of magnitude finer than any physically relevant scale.
    S_ice_rows = S_ice[has_crossing]    # (n_kept, n_scan), one row per kept P

    def _interp_ice_S(T_arr):
        """1D linear interp of S_ice_rows[i] at T_arr[i] for each kept row."""
        j = np.searchsorted(T_scan, T_arr) - 1
        j = np.clip(j, 0, n_scan - 2)
        t0 = T_scan[j]
        t1 = T_scan[j + 1]
        # Gather row-specific endpoints (np broadcasting via take_along_axis)
        s0 = np.take_along_axis(S_ice_rows, j[:, None], axis=1).ravel()
        s1 = np.take_along_axis(S_ice_rows, (j + 1)[:, None], axis=1).ravel()
        return s0 + (s1 - s0) * (T_arr - t0) / (t1 - t0)

    S_lo_liq = compute_batch(T_lo_b, P_b)[S_key]
    S_lo_ice = _interp_ice_S(T_lo_b)
    sign_lo = np.sign(S_lo_liq - S_lo_ice)

    # 30 bisection iterations. Liquid evaluated exactly each step; ice via
    # the pre-built interpolant. Removes 30 SeaFreeze scatter calls.
    for _ in range(30):
...
def _get_compute_at_x(model_key):
    ...
    try:
        if model_key == 'duska2020':
            ...
            from caupin_eos.core import compute_properties_at_x as _cx
            from caupin_eos import params_kim as _pk

            def compute_properties_at_x(T_K, p_MPa, x, _compute_Kp=False):
                return _cx(T_K, p_MPa, x, _compute_Kp=_compute_Kp, pset=_pk)
        else:
            return None
