...
def _to_reduced(T_K, p_MPa, pset=_DEFAULT_PARAMS):
...
def _get_adapter(pset=_DEFAULT_PARAMS):
...
def find_LLCP(pset=_DEFAULT_PARAMS):
    """
    ...
    def _GBA_at_critical(dPh):
        ...
        if Th is None:
            return float('inf')
        dTh = Th - 1.0
        return compute_GBA(dTh, dPh, pset)[0]
...
def _get_compute_batch(pset=_DEFAULT_PARAMS):
    # TMD/Kauzmann helpers need a vectorized compute_batch.
    import caupin_eos.params_kim as _pk
    if pset is _pk:
        # With-Kim (Table II): prefer the Rust batch, fall back to numpy.
        try:
            from watereos_rs import compute_batch_caupin_kim
            return compute_batch_caupin_kim
        except ImportError:
            from .core import compute_batch as _cb
            return lambda T, p: _cb(T, p, pset=_pk)
    # Default (Table III): unchanged — shared dispatch (Rust-preferred).
...
def _rust_phase_funcs(pset):
    """Return (spinodal_fn, binodal_fn) from watereos_rs matching the param
    set, or (None, None) if the Rust backend is unavailable."""
    import caupin_eos.params_kim as _pk
    try:
        import watereos_rs as _rs
    except ImportError:
        return None, None
    if pset is _pk:
        return _rs.compute_spinodal_caupin_kim, _rs.compute_binodal_caupin_kim
    return _rs.compute_spinodal_caupin, _rs.compute_binodal_caupin


def compute_spinodal_curve(p_range_MPa=None, n_pressures=150, pset=_DEFAULT_PARAMS):
    ...
    rust_spin, _ = _rust_phase_funcs(pset)
    if rust_spin is not None:
        return rust_spin(p_range_MPa, llcp['T_K'], llcp['p_MPa'])
    return _spinodal(_get_adapter(pset), p_range_MPa, n_pressures)


def compute_binodal_curve(p_range_MPa=None, n_pressures=150, spinodal=None,
                          pset=_DEFAULT_PARAMS):
    ...
    p_range_MPa = np.asarray(p_range_MPa, dtype=float)
    if spinodal is None:
        spinodal = compute_spinodal_curve(p_range_MPa, n_pressures, pset)
    _, rust_bino = _rust_phase_funcs(pset)
    if rust_bino is not None:
        return rust_bino(
            spinodal['p_array'], spinodal['T_upper'], spinodal['T_lower'],
            llcp['T_K'], llcp['p_MPa'])
    return _binodal(_get_adapter(pset), p_range_MPa, n_pressures, spinodal)


def compute_phase_diagram(p_range_MPa=None, n_pressures=150, pset=_DEFAULT_PARAMS):
...
def compute_tmd_temperature(P_MPa, pset=_DEFAULT_PARAMS, **kwargs):
    """Compute TMD temperature (alpha=0) at given pressure(s)."""
    return _tmd(P_MPa, _get_compute_batch(pset), **kwargs)


def compute_kauzmann_temperature(P_MPa, pset=_DEFAULT_PARAMS, **kwargs):
    """Compute Kauzmann temperature (S_liquid = S_ice_Ih) at given pressure(s)."""
    return _kauz(P_MPa, _get_compute_batch(pset), **kwargs)
