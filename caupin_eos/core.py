...
import math
from . import params as _DEFAULT_PARAMS
...
def spinodal_pressure(T_K, pset=_DEFAULT_PARAMS):
    """
...
def compute_Gsigma(dTh, dPh, T_K, pset=_DEFAULT_PARAMS):
    """
    ...
    Ps, dPs_dT, d2Ps_dT2 = spinodal_pressure(T_K, pset)
    Phs = Ps / P.P_scale_MPa

    # u = P̂ - P̂s = (ΔP̂ + P̂c) - P̂s = ΔP̂ + (Pc - Ps)/P_scale
    # where P̂ = P/P_scale = ΔP̂ + Pc/P_scale
    dPhs = (P.Pc - Ps) / P.P_scale_MPa  # P̂c - P̂s
    u = dPh + dPhs  # = P̂ - P̂s

    # Derivatives of dPhs w.r.t. ΔT̂ (chain rule: ∂/∂ΔT̂ = Tc·∂/∂T)
    # ∂P̂s/∂ΔT̂ = (dPs/dT)·Tc / P_scale
    dPhs_dTh = dPs_dT * P.Tc / P.P_scale_MPa
    d2Phs_dTh2 = d2Ps_dT2 * P.Tc**2 / P.P_scale_MPa
    ...
    # Below the liquid-vapor spinodal (u <= 0) the spinodal Gibbs contribution
    # is undefined: propagate NaN rather than silently returning unphysical values.
    if u <= 0.0:
        nan = float('nan')
...
def compute_GA_poly(dTh, dPh, pset=_DEFAULT_PARAMS):
    """
...
def compute_GA(dTh, dPh, T_K, pset=_DEFAULT_PARAMS):
    """
    Full Ĝ^A = Ĝ^σ + Σ c_mn ΔT̂^m ΔP̂^n and all derivatives.
    """
    Gs, dGs_dP, dGs_dT, d2Gs_dP2, d2Gs_dT2, d2Gs_dPdT = \
        compute_Gsigma(dTh, dPh, T_K, pset)
    Gp, dGp_dP, dGp_dT, d2Gp_dP2, d2Gp_dT2, d2Gp_dPdT = \
        compute_GA_poly(dTh, dPh, pset)
...
def compute_GBA(dTh, dPh, pset=_DEFAULT_PARAMS):
    """
...
def compute_omega(dTh, dPh, pset=_DEFAULT_PARAMS):
    """
...
def _physical_props(Vh, Sh_red, d2G_dP2_total, d2G_dT2_total,
                    d2G_dPdT_total, T_K, G_hat_red=0.0, pset=_DEFAULT_PARAMS):
    """
...
def compute_properties(T_K, p_MPa, _compute_Kp=False, pset=_DEFAULT_PARAMS):
    """
    ...
    mix = _physical_props(Vh, Sh_red, d2G_dP2_total, d2G_dT2_total,
                          d2G_dPdT_total, T_K, G_hat_mix, pset)
    mix['x'] = x

    # ── State A properties (x = 0, no mixing terms) ───────────────────
    stateA = _physical_props(dGA_dP, -dGA_dT,
                             d2GA_dP2, d2GA_dT2, d2GA_dPdT, T_K, G_hat_A, pset)
    ...
    stateB = _physical_props(VhB, ShB_red,
                             d2GB_dP2, d2GB_dT2, d2GB_dPdT, T_K, G_hat_B, pset)
    ...
    if _compute_Kp:
        dp = 0.001  # MPa
        props_plus = compute_properties(T_K, p_MPa + dp, pset=pset)
        props_minus = compute_properties(T_K, p_MPa - dp, pset=pset)
        for suffix in ['', '_A', '_B']:
            result['Kp' + suffix] = (
...
def compute_properties_at_x(T_K, p_MPa, x, _compute_Kp=False, pset=_DEFAULT_PARAMS):
    """
    ...
    if _compute_Kp:
        dp = 0.001  # MPa
        pp = compute_properties_at_x(T_K, p_MPa + dp, x, pset=pset)
        pm = compute_properties_at_x(T_K, p_MPa - dp, x, pset=pset)
...
def compute_batch(T_K, p_MPa, pset=_DEFAULT_PARAMS):
    """
    ...
    pset  : module — parameter set (defaults to the without-Kim Table III fit;
            pass caupin_eos.params_kim for the with-Kim Table II fit).
    ...
    with np.errstate(invalid='ignore'):
        u_safe = np.where(u > 0.0, u, np.nan)
        sqrt_u = np.sqrt(u_safe)
        u32 = u_safe * sqrt_u
        u12 = sqrt_u
        u_m12 = 1.0 / sqrt_u  # NaN propagates naturally
    ...
    def _phys_vec(Vh_v, Sh_red_v, d2P2, d2T2, d2PT, G_hat_v):
        ...
        # Preserve NaN (e.g. below liquid-vapor spinodal) instead of falling back to inf.
        with np.errstate(divide='ignore', invalid='ignore'):
            rho = np.where(np.isnan(V_spec), np.nan,
                           np.where(V_spec > 0, 1.0 / V_spec, np.inf))
        ...
        # NaN-aware guards: propagate NaN inputs (e.g. below liquid-vapor spinodal)
        # while preserving existing behavior for finite-but-out-of-domain cases.
        nan_mask = np.isnan(Vh_v) | np.isnan(d2P2) | np.isnan(d2T2) | np.isnan(d2PT)

        kappa_T = np.where(np.abs(Vh_v) > 1e-30,
                           -(P.Vc / (P.R * P.Tc)) * d2P2 / Vh_v,
                           np.inf)
        Kt_v = np.where(nan_mask, np.nan,
                        np.where((np.abs(kappa_T) > 1e-30) & np.isfinite(kappa_T),
                                 1.0 / kappa_T / 1e6, 0.0))

        alpha_v = np.where(nan_mask, np.nan,
                           np.where(np.abs(Vh_v) > 1e-30,
                                    (1.0 / P.Tc) * d2PT / Vh_v, 0.0))

        Cv_v = np.where(nan_mask, np.nan,
                        np.where((kappa_T > 0) & np.isfinite(kappa_T),
        ...
        kappa_S = np.where(Cp_v > 0,
                           ...
                           kappa_T)
        Ks_v = np.where(nan_mask, np.nan,
                        np.where(kappa_S > 0, 1.0 / kappa_S / 1e6, np.inf))

        vel_v = np.where(nan_mask, np.nan,
                         np.where((rho > 0) & (kappa_S > 0),
